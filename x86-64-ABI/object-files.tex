...
\item[L] Represents the value of the symbol whose index resides in the
  relocation entry.  The symbol value is the place (section offset or
  address) of the \textindex{Procedure Linkage Table} entry for the
  symbol when it is defined externally in a shared object.
...
\begin{sloppypar}
The \texttt{R_X86_64_PLT32} relocation is different from the equivalent
i386 \texttt{R_386_PLT32} relocation.  On i386, there are 2 types of
PLTs (\textindex{Procedure Linkage Table}), PIC and non-PIC.  Shared
objects must use PIC PLT which requires that \texttt{_GLOBAL_OFFSET_TABLE_}
must be loaded into EBX register first.  The \xARCH architecture only
has one PLT which is PC-relative. The \texttt{R_X86_64_PLT32} relocation,
instead of the \texttt{R_X86_64_PC32} relocation, should be used with
32-bit PC-relative branches to global functions.  Branches to local
functions should use the \texttt{R_X86_64_PC32} relocation.
\end{sloppypar}
